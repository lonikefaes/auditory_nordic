...
Beta_Std = xff([dirdata, subj, '_Betas_SingleTrial_Oddball_L_bn_11-DepthLevels.smp']);                  % We have sampled the data at 11 different depths in BV
Beta_NOR = xff([dirdata, subj, '_Betas_SingleTrial_Oddball_L_an_11-DepthLevels.smp']);
Beta_NN = xff([dirdata, subj, '_Betas_SingleTrial_Oddball_L_an_NoNoise_11-DepthLevels.smp']);


poi_LH = xff([dirpoi, subj, '_ROI_LH_no_overlap.poi']);                    % Load the poi of one hemisphere.
HG_LH = poi_LH.POI(1).Vertices;   

Betas = zeros( size(HG_LH,1), nrMaps);
Betas_NOR = zeros( size(HG_LH,1), nrMaps);
Betas_NN = zeros( size(HG_LH,1), nrMaps);
...
%% Depth 1  - for each dataset and each depth we have the following:
D1_Betas = Betas(:,D1);                         % Betas in percent signal change all voxels and trials
D1_mean = mean(D1_Betas);                       % Calculate mean across voxels
D1_se = std(D1_mean)/sqrt(length(D1_mean));     % Standard error --> variance across trials
...
folderLam = 'D:\LaminarfMRI_Audio\MN\NoGap\Post-Covid\ResultsGroupAnalysis\LaminarData\';
%saveas(h,[folderOut, subj, '_LaminarPlot.fig']);
%saveas(h, [folderOut, subj,'_LaminarPlot.svg']);
%saveas(h, [folderOut, subj, '_LaminarPlot.png']);
save([folderLam, [subj, '_LaminarPlot_OddL.mat']],'x', 'y', 'z','std_dev', 'std_dev_NOR', 'std_dev_NN');
