...
subj       = 'S11_MN_NG_PC';     % Change participant number accordingly
...
%% Create figure of Spatial Correlations --> Individual subject data used in Figures 3 and S3 and S4
...
%% here we compute the information regarding the design matrix before NORDIC and after and in the residuals - leaking --> used in Figures 7 and S11 and S12
...
%% Understanding variance partitioning - this was for us checking the variance partitioning, this not an analysis specified in the paper.
% voxsel = 12003;
...
Ran_vmp = [Rbres_refSSY]';
RanNS_vmp = [RbresNS_refSSY]';
...
%% Betas and t consistency --> per ROI --> Figures 5 and S5, S6, S7, S8 and S9.
connames             = {'Comp_H' , 'Comp_L', 'Oddball_L','Oddball_H', 'Unexp_H', 'Unexp_L' };% names of the contrasts
...
%% Difference in beta --> Figure 6 and S10.
...
h = gcf;
% saveas(h,[folderOut, [subj,'_SumOfS_Model_Vertical.fig']]);
%save([folderOut,[subj, '_SumOfS_Model_Vertical.mat']],'Rbn','Ran','Rbres','RanNS','RbresNS', 'Rbres_refSSY', 'RbresNS_refSSY', 'tsnrBN', 'BetasCond');
% saveas(h, [folderOut, [subj,'_SumOfS_Model_Vertical.svg']]);
% saveas(h, [folderOut, [subj,'_SumOfS_Model_Vertical.png']]);
...
[my binsx ] = window_mean(  squeeze(BetasCond(it,1,:)) - squeeze(BetasCond(it,2,:)),  mean(tsnrBN), linspace(5,60,30) );
plot(my,binsx,'r-','LineWidth',1)
%xlabel('$\beta$','interpreter','Latex', 'fontsize', 12);
axis square;
la = '$\frac{Mean}{Std}$';
ylabel(la,'interpreter','Latex', 'fontsize', 20, 'fontname', 'Arial')
xlim([-10 10]);
...
[my binsx ] = window_mean(  squeeze(BetasCond(it,1,:)) - squeeze(BetasCond(it,3,:)),   mean(tsnrBN), linspace(5,60,30) );
plot(my,binsx,'r-','LineWidth',1)
%xlabel('$\beta$','interpreter','Latex', 'fontsize', 12);
axis square;
la = '$\frac{Mean}{Std}$';
